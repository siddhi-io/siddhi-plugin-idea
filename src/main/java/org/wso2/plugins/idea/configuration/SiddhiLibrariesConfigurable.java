...
public class SiddhiLibrariesConfigurable implements SearchableConfigurable, Configurable.NoScroll {
    ...
    private final JBCheckBox myUseEnvSiddhiPathCheckBox = new JBCheckBox("Use SIDDHI_REPOSITORY that's " +
    ...
    public SiddhiLibrariesConfigurable(@NotNull String displayName, @NotNull SiddhiLibrariesService
        ...
        filesList.setCellRenderer(new ColoredListCellRenderer() {
            ...
            protected void customizeCellRenderer(@NotNull JList list, Object value, int index, boolean selected,
                ...
                String url = item.url;
                if (item.readOnly) {
                    append("[SIDDHI_REPOSITORY] ", SimpleTextAttributes.GRAY_ATTRIBUTES);
                }
        ...
        if (librariesService instanceof SiddhiApplicationLibrariesService) {
            myUseEnvSiddhiPathCheckBox.addActionListener(event -> {
                if (myUseEnvSiddhiPathCheckBox.isSelected()) {
                    addReadOnlyPaths();
                } else {
                    removeReadOnlyPaths();
                }
            });
            myPanel.add(myUseEnvSiddhiPathCheckBox, BorderLayout.SOUTH);
        }
    }
    ...
    public boolean isModified() {
        return !getUserDefinedUrls().equals(myLibrariesService.getLibraryRootUrls()) ||
                myLibrariesService instanceof SiddhiApplicationLibrariesService &&
                        ((SiddhiApplicationLibrariesService) myLibrariesService)
                                .isUseSiddhiPathFromSystemEnvironment() !=
                                myUseEnvSiddhiPathCheckBox.isSelected();
    }

    @Override
    public void apply() throws ConfigurationException {
        myLibrariesService.setLibraryRootUrls(getUserDefinedUrls());
        if (myLibrariesService instanceof SiddhiApplicationLibrariesService) {
            ((SiddhiApplicationLibrariesService) myLibrariesService).setUseSiddhiPathFromSystemEnvironment
                    (myUseEnvSiddhiPathCheckBox.isSelected());
        }
    }
    ...
    private void resetLibrariesFromEnvironment() {
        if (myLibrariesService instanceof SiddhiApplicationLibrariesService) {
            myUseEnvSiddhiPathCheckBox.setSelected(((SiddhiApplicationLibrariesService) myLibrariesService)
                    .isUseSiddhiPathFromSystemEnvironment());
            if (((SiddhiApplicationLibrariesService) myLibrariesService).isUseSiddhiPathFromSystemEnvironment()) {
                addReadOnlyPaths();
            } else {
                removeReadOnlyPaths();
            }
    ...
    public void disposeUIResources() {
        UIUtil.dispose(myUseEnvSiddhiPathCheckBox);
