    ...
    private Configurable createConfigurable(boolean dialogMode) {
        return new CompositeConfigurable<UnnamedConfigurable>() {
            ...
            protected List<UnnamedConfigurable> createConfigurables() {
                ...
                String[] urlsFromEnv =
                        ContainerUtil.map2Array(SiddhiSdkUtil.getSiddhiPathsRootsFromEnvironment(), String.class,
                                VirtualFile::getUrl);
                result.add(new SiddhiLibrariesConfigurable("Global libraries",
                        SiddhiApplicationLibrariesService.getInstance(), urlsFromEnv));
                if (!myProject.isDefault()) {
                    result.add(new SiddhiLibrariesConfigurable("Project libraries",
                            SiddhiProjectLibrariesService.getInstance(myProject)));
                }
                return result;
            }
